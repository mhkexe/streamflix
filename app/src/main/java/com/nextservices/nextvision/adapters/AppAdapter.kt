...
import com.nextservices.nextvision.adapters.viewholders.CategoryViewHolder
import com.nextservices.nextvision.adapters.viewholders.EpisodeViewHolder
import com.nextservices.nextvision.adapters.viewholders.GenreViewHolder
import com.nextservices.nextvision.adapters.viewholders.MovieViewHolder
import com.nextservices.nextvision.adapters.viewholders.PeopleViewHolder
import com.nextservices.nextvision.adapters.viewholders.ProviderViewHolder
import com.nextservices.nextvision.adapters.viewholders.SeasonViewHolder
import com.nextservices.nextvision.adapters.viewholders.TvShowViewHolder
...
import com.nextservices.nextvision.databinding.ContentTvShowCastMobileBinding
import com.nextservices.nextvision.databinding.ContentTvShowCastTvBinding
import com.nextservices.nextvision.databinding.ContentTvShowDirectorsMobileBinding
import com.nextservices.nextvision.databinding.ContentTvShowDirectorsTvBinding
import com.nextservices.nextvision.databinding.ContentTvShowMobileBinding
import com.nextservices.nextvision.databinding.ContentTvShowRecommendationsMobileBinding
import com.nextservices.nextvision.databinding.ContentTvShowRecommendationsTvBinding
import com.nextservices.nextvision.databinding.ContentTvShowSeasonsMobileBinding
import com.nextservices.nextvision.databinding.ContentTvShowSeasonsTvBinding
import com.nextservices.nextvision.databinding.ContentTvShowTvBinding
...
import com.nextservices.nextvision.databinding.ItemFavoriteSectionHeaderBinding
import com.nextservices.nextvision.databinding.ItemGenreGridMobileBinding
import com.nextservices.nextvision.databinding.ItemGenreGridTvBinding
import com.nextservices.nextvision.databinding.ItemLoadingBinding
...
import com.nextservices.nextvision.databinding.ItemPeopleMobileBinding
import com.nextservices.nextvision.databinding.ItemPeopleTvBinding
...
import com.nextservices.nextvision.fragments.favorites.FavoriteSectionHeader
...
class AppAdapter(
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        when (holder) {
            is CategoryViewHolder -> holder.bind(
                items[dataPosition(position)] as Category,
            ...
            is EpisodeViewHolder -> holder.bind(
                items[dataPosition(position)] as Episode
            ...
            is FavoriteSectionHeaderViewHolder -> holder.bind(
                items[dataPosition(position)] as FavoriteSectionHeader
            )
            is GenreViewHolder -> holder.bind(
                items[dataPosition(position)] as Genre
            ) // Tu original no pasaba listener, lo respeto
            is HeaderViewHolder -> header?.bind?.invoke(holder.binding)
            is MovieViewHolder -> holder.bind(
                items[dataPosition(position)] as Movie,
                onMovieClickListener,
                onMovieLongClickListener,
                onMovieKeyListener,
                isItemSelectedListener?.invoke(items[dataPosition(position)]) == true,
            ) // Los listeners se manejan dentro del ViewHolder
            is PeopleViewHolder -> holder.bind(
                items[dataPosition(position)] as People
            ) // Tu original no pasaba listener, lo respeto
            is ProviderViewHolder -> holder.bind(
                items[dataPosition(position)] as Provider
            ) // Tu original no pasaba listener, lo respeto
            is SeasonViewHolder -> holder.bind(
                items[dataPosition(position)] as Season
            ) // Tu original no pasaba listener, lo respeto
            is TvShowViewHolder -> holder.bind(
                items[dataPosition(position)] as TvShow,
                onTvShowClickListener,
                onTvShowLongClickListener,
                onTvShowKeyListener,
                isItemSelectedListener?.invoke(items[dataPosition(position)]) == true,
    ...
    override fun getItemCount(): Int = if (isLooping && items.isNotEmpty()) {
    ...
    private fun dataPosition(position: Int): Int {
        val adjustedPosition = header?.let { position - 1 } ?: position
        return if (isLooping && items.isNotEmpty()) {
            adjustedPosition % items.size
        } else {
            adjustedPosition
        }
    }
