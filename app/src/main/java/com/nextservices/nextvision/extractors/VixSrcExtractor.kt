        ...
        try {
            ...
            client.newCall(request).execute().use { response ->
                if (response.isSuccessful && response.body != null) {
                    ...
                    Log.i("NextVisionES", "[VixSrc] --- Processing START (Lang: $langCode) ---")
                    ...
                    for (line in lines) {
                        ...
                        if (patchedLine.startsWith("#EXT-X-MEDIA:TYPE=AUDIO")) {
                            ...
                            if (isTargetAudio) {
                                ...
                                Log.i("NextVisionES", "[AUDIO] -> SET DEFAULT: $langCode")
                            }
                            ...
                            if (isForced && isRightLanguage) {
                                patchedLine = patchedLine.replace("DEFAULT=NO", "DEFAULT=YES")
                                                         .replace("AUTOSELECT=NO", "AUTOSELECT=YES")
                                Log.i("NextVisionES", "[SUBTITLE] -> ENABLED FORCED: $trackName")
                            }
                            finalLines.add(patchedLine)
                        } else {
                            finalLines.add(patchedLine)
                    ...
                    Log.i("NextVisionES", "[VixSrc] --- Processing END ---")
